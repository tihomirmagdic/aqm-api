"use strict";
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (Object.hasOwnProperty.call(mod, k)) result[k] = mod[k];
    result["default"] = mod;
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
const sqlProvider = require("../sql");
// import _ from "lodash";
const Joi = __importStar(require("@hapi/joi"));
const default_schemas_1 = require("../../server/default-schemas");
// id, name
// schemas
exports.shConfigurationsCreate = Joi.object().keys({
    id: Joi.string().required(),
    name: Joi.string().required(),
});
exports.shConfigurationsValues = Joi.object().keys({
    id: Joi.string(),
    name: Joi.string(),
});
exports.shConfigurationsUpdate = Joi.object().keys({
    ids: Joi.array().items(default_schemas_1.shText).required(),
    values: exports.shConfigurationsValues.required(),
});
const sql = sqlProvider.configurations;
class ConfigurationsRepository {
    constructor(db, pgp) {
        this.db = db;
        this.pgp = pgp;
    }
    get() {
        return this.db.any(sql.getAll);
    }
    getByIDs(where) {
        return this.db.any(sql.getByIDs, { where });
    }
    add(type, values) {
        if (type === "full") {
            return this.db.task("add configuration", (t) => this.add("id", values).then((id) => {
                console.log("id:", id);
                return this.getByIDs([id]);
            }));
        }
        const colValues = this.pgp.helpers.values(values);
        console.log("colValues:", colValues);
        const { dbcall, returning } = (type === "id") ?
            { dbcall: this.db.one, returning: "returning id" } : // return one record with id column
            { dbcall: this.db.none, returning: "" }; // call SQL insert, and do not return any record
        return dbcall(sql.add, { values, colValues, returning });
    }
    update(type, data) {
        if (type === "full") {
            return this.db.task("update configurations", (t) => this.update("fast", data).
                then(() => {
                if (data.values.id) {
                    data.ids.map((item) => item.id = data.values.hasOwnProperty("id") ? data.values.id : item.id);
                }
                // console.log("before find:", data.ids);
                return this.getByIDs(data.ids);
            }));
        }
        else {
            const where = data.ids;
            const set = this.pgp.helpers.sets(data.values);
            return this.db.result(sql.update, { set, where }, (r) => ({ updated: r.rowCount }));
        }
    }
    delete(where) {
        return this.db.result(sql.remove, { where }, (r) => ({ deleted: r.rowCount }));
    }
}
exports.ConfigurationsRepository = ConfigurationsRepository;
//# sourceMappingURL=configurations copy.js.map