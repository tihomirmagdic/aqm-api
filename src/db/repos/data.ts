...
export class DataRepository {
  ...
  private replaceField = (values: string[], field: string, newField: string) => {
  ...
  private replaceFields = (values: string[], fields: string[], newFields: string[]) =>
    fields.forEach((field: string, i: number) => this.replaceField(values, field, newFields[i]));

  private replaceAllFieldsRule = (values: string[], fields: string[], prefix: string, postfix: string, exceptFields: string[]) => {
    const rFields = fields.filter((field: string) => !exceptFields.includes(field));
    const newFields = rFields.map((field: string) => prefix + field + postfix + ` as ${field}`);
    ...
    */
    ...
    if (values.locations.devices)  {
      ...
      if (values.locations.name) {
        ...
        locations = `r.id = '${values.locations.name}' and st_within(gps, r.area)`;
        ...
        const sPolygon = 
          ...
          polygon.map((pt: any) => `ST_SetSRID(ST_MakePoint(${pt[0]}, ${pt[1]}), 4326)`)
          .join(", ") +
          "]))";
    ...
    const order = (values.order ? `\norder by ` + values.order.map((field: string) => field.substr(0, 1) === "-" ? field.substr(1) + " desc" : field).join(", ") : "") + "\n";
  ...
  private createColumnsets() {
    ...
    if (!DataRepository.cs) {
      const helpers = this.pgp.helpers, cs: DataColumnsets = {};

      const colGPS = new this.pgp.helpers.Column({
        name: "gps", // required
        mod: ":raw", // optional
        init: (params: any) => this.pgp.as.format(`ST_SetSRID(ST_MakePoint(${params.value[0]}, ${params.value[1]}), 4326)`)
      });
