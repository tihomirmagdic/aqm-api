...
// schemas
export const shRegionTypesCreate = Joi.object().keys({
  id: Joi.string().required(),
  name: Joi.string().required(),
  parent: Joi.string().allow(null)
});

export const shRegionTypesValues = Joi.object().keys({
  id: Joi.string(),
  name: Joi.string(),
  parent: Joi.string()
});

export const shRegionTypesUpdate = Joi.object().keys({
  ids: Joi.array().items(shText).required(),
  values: shRegionTypesValues.required(),
});

const sql = sqlProvider.regiontypes;

export class RegionTypesRepository {
