...
// schemas
export const shFirmwaresCreate = Joi.object().keys({
  name: Joi.string().required(),
  bin: Joi.string().required()
});

export const shFirmwaresValues = Joi.object().keys({
  name: Joi.string(),
  bin: Joi.string()
});

export const shFirmwaresUpdate = Joi.object().keys({
  ids: Joi.array().items(shID).required(),
  values: shFirmwaresValues.required(),
});

const sql = sqlProvider.firmwares;

export class FirmwaresRepository {
